...
from COVID19_Chart.config import SUMMARY_FILE_PATH, FILE_TYPE, FILE_LOCATION, STATES, LATEST_UPDATE, TODAY

def dateRange(start_date, end_date):
    for n in range(int((end_date - start_date).days)):
        yield start_date + datetime.timedelta(n)

def setSummary(file_type, file_location, target_file=SUMMARY_FILE_PATH, start_date = LATEST_UPDATE, end_date = TODAY):
...
def consolodate(start_date=LATEST_UPDATE, end_date=TODAY):
    for state in STATES:
